...
class _ModelStdVert():
	def __init__(self, section, file_object):
		...
		self.ModelStdVertLength = section.length
...
#filePath = r"C:\Users\Xavier\Downloads\9870FFAD9BAF955A_hero_peterparker_head.model"
...
#	colorList = []
#	f.seek(ModelColVert.ModelColVertOffset + modelSubset.vertexIndex * 0x04)
#	for y in range(modelSubset.vertexCount):
#		cr = read_byte(f) / 255.0
#		cg = read_byte(f) / 255.0
#		cb = read_byte(f) / 255.0
#		ca = read_byte(f) / 255.0

#		colorList.append([cr,cg,cb,ca])
...
#	color_layer = new_mesh.vertex_colors.active.data
#	for loop in new_mesh.loops:
#		color_layer[loop.index].color = colorList[loop.vertex_index]
